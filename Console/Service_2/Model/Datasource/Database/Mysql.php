<?php
	...
	class MysqlAdapter extends PDO {
		...
		private $__defaultSelectOptions;
		private $__defaultInsertOptions;
		private $__defaultDeleteOptions;
		private $__defaultUpdateOptions;
		private $__conditionsOperators;
		...
		public function __construct($host, $db, $user, $pwd) {
			...
			$this->__defaultSelectOptions = array(
				'fields' => array('*'),
				'table' => array()
			);
			$this->__defaultInsertOptions = array(
				'table' => '',
				'values' => array()
			);
			$this->__defaultDeleteOptions = array(
				'table' => ''
			);
			$this->__defaultUpdateOptions = array(
				'table' => '',
				'set' => array()
			);
			$this->__conditionsOperators = array(
				'AND', 
				'OR',
				'NOT'
			);
		...
		public function query($query, $param = array()) {
			...
			if($this->queryIsSelect($query)) {
				$result = $this->__fetchAll($pdoStatement);
			}
			else {
		...
		public function __fetchAll($statement) {
			$result = array();
		...
		public function buildSelect($opt) {
			...
			if(isset($opt['for_update']) && $opt['for_update'] == true) {
