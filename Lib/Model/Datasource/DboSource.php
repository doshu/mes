 ...
 */
	public function buildStatement($query, $model) {
		...
		return $this->renderStatement('select', array(
			...
			'having' => $this->having($query['having'], $model),
			'extra' => implode(' ', (array)$query['extra'])
		));
	}
 ...
 */
	...
	public function having($data, $model) {
		if(!empty($data)) {
			$having = $this->conditions($data, true, false, $model);
			if(!empty($having)) {
 ...
 */
	public function renderStatement($type, $data) {
		...
		switch (strtolower($type)) {
			case 'select':
				return "SELECT {$fields} FROM {$table} {$alias} {$joins} {$conditions} {$group} {$having} {$order} {$limit} {$extra}";
			case 'create':
 ...
 */
	protected function _scrubQueryData($data) {
		static $base = null;
		if ($base === null) {
			$base = array_fill_keys(array('conditions', 'fields', 'joins', 'order', 'limit', 'offset', 'group', 'having', 'extra'), array());
			$base['callbacks'] = null;
		}
